...
function main()
    ...
    runningTime_ch_32_Directions = Matrix{Float32}(undef, length(instanceNames), 7)
    ...
    for i in 1:length( data_sizes )
        for j in 1:setNumbers
            ...
            if benchmarking
                ...
                bm_ch_32_Directions = run(@benchmarkable ch_32_Directions($points) samples=25 seconds=10000)
                runningTime_ch_32_Directions[t,:] = report(bm_ch_32_Directions, 5)

            else

                exportFile_ch_32_Directions = string(resultDirectory, instanceNames[t], "_ch_32_Directions")
                ch_32_Directions(points, exportResult, exportFile_ch_32_Directions)
    ...
    if benchmarking
        ...
        exportReport(instanceNames, runningTime_ch_32_Directions, string(resultDirectory,"ch_32_Directions_running_time.csv"))
