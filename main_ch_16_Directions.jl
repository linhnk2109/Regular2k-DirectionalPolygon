...
function main()
    ...
    runningTime_ch_16_Directions = Matrix{Float32}(undef, length(instanceNames), 7)
    ...
    for i in 1:length( data_sizes )
        for j in 1:setNumbers
            ...
            if benchmarking
                ...
                runningTime_ch_16_Directions[t,:] = report(bm_ch_16_Directions, 5)

            else

                exportFile_ch_16_Directions = string(resultDirectory, instanceNames[t], "_ch_16_Directions")
                ch_16_Directions(points, exportResult, exportFile_ch_16_Directions)
    ...
    if benchmarking
        ...
        exportReport(instanceNames, runningTime_ch_16_Directions, string(resultDirectory,"ch_16_Directions_running_time.csv"))
