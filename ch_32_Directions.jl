...
function find2kExtremePoints(points::Matrix{Float64})
    @timeit TimeOutput "find 32 ex" begin
        kDrection = 16
    ...
    return [ex[1], ex[3], ex[5], ex[7], ex[9], ex[11], ex[13], ex[15],
            ex[17], ex[19], ex[21], ex[23], ex[25], ex[27], ex[29], ex[31],
            ex[2],ex[4], ex[6], ex[8], ex[10],ex[12], ex[14], ex[16],
            ex[18],ex[20], ex[22], ex[24], ex[26],ex[28], ex[30], ex[32],ex[1]]
end
...
function ch_32_Directions(points, exportCH = false, exportFile = "")
